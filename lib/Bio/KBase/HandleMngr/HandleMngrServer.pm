package Bio::KBase::HandleMngr::HandleMngrServer;
...
our %method_authentication = (
        'is_readable' => 'optional',
        'add_read_acl' => 'required',
        'set_public_read' => 'required',
);
...
sub call_method {
    ...
    my $ctx = HandleMngrServerContext->new($self->{loggers}->{userlog},
...
{
    # Service HandleMngr requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for HandleMngr but no authentication header was passed");
	}
    ...
    my @result;
    {
        ...
        my $stderr = HandleMngrServerStderrWrapper->new($ctx, $get_time);
        $ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service HandleMngr.");
    }
...
package HandleMngrServerContext;

use strict;

=head1 NAME

HandleMngrServerContext
...
package HandleMngrServerStderrWrapper;
...
unless (caller) {
    ...
    my $server = HandleMngrServer->new(
